...
// Individual test: JC addr -- jumps if carry=1. Forced with ADD A on 255
// (255+255 overflows 8 bits -> carry=1).
module test_jc;
    reg clk = 0;
    reg reset = 0;
    reg program_mode = 0;
    ...
    initial begin
        ...
        prog[0] = 8'h08; prog[1] = 8'd20; // LD A,#20   A=255
        prog[2] = 8'h18;                  // ADD A      A=254, carry=1
        prog[3] = 8'h53; prog[4] = 8'd9;  // JC #9      should jump
        ...
        if (external_output == 8'd42)
            $display("PASS JC: external_output = %0d (expected 42)", external_output);
        else
            $display("FAIL JC: external_output = %0d (expected 42)", external_output);
