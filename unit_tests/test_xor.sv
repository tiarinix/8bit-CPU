...
// Individual test: XOR reg -- A <- A ^ reg
module test_xor;
    reg clk = 0;
    reg reset = 0;
    reg program_mode = 0;
    ...
    initial begin
        ...
        prog[0] = 8'h08; prog[1] = 8'd10; // LD A,#10   A=0xFF
        prog[2] = 8'h09; prog[3] = 8'd11; // LD B,#11   B=0x0F
        prog[4] = 8'h39;                  // XOR B      A=0xFF^0x0F=0xF0
        prog[5] = 8'h40;                  // OUT A
        prog[6] = 8'hFF;                  // HLT
        prog[10] = 8'hFF;
        prog[11] = 8'h0F;
        ...
        if (external_output == 8'd240)
            $display("PASS XOR: external_output = %0d (expected 240)", external_output);
        else
            $display("FAIL XOR: external_output = %0d (expected 240)", external_output);
