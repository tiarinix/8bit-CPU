...
// Individual test: JNZ addr -- jumps if zero=0. Forced with ADD A (5+5=10, not zero).
module test_jnz;
    reg clk = 0;
    reg reset = 0;
    reg program_mode = 0;
    ...
    initial begin
        ...
        prog[2] = 8'h18;                  // ADD A      A=10, zero=0
        prog[3] = 8'h52; prog[4] = 8'd9;  // JNZ #9     should jump
        ...
        if (external_output == 8'd42)
            $display("PASS JNZ: external_output = %0d (expected 42)", external_output);
        else
            $display("FAIL JNZ: external_output = %0d (expected 42)", external_output);
