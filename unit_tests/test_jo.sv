...
// Individual test: JO addr -- jumps if overflow=1. Same case as JN
// (127+127: two positives give a negative result -> overflow=1).
module test_jo;
    reg clk = 0;
    reg reset = 0;
    reg program_mode = 0;
    ...
    initial begin
        ...
        prog[2] = 8'h18;                  // ADD A      A=254, overflow=1
        prog[3] = 8'h56; prog[4] = 8'd9;  // JO #9      should jump
        ...
        if (external_output == 8'd42)
            $display("PASS JO: external_output = %0d (expected 42)", external_output);
        else
            $display("FAIL JO: external_output = %0d (expected 42)", external_output);
