...
// Individual test: LD reg,addr -- reg <- mem[addr]
module test_ld;
    reg clk = 0;
    reg reset = 0;
    reg program_mode = 0;
    ...
    initial begin
        ...
        if (external_output == 8'd42)
            $display("PASS LD: external_output = %0d (expected 42)", external_output);
        else
            $display("FAIL LD: external_output = %0d (expected 42)", external_output);
